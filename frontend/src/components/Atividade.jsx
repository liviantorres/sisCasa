...
const ContainerInformacoesCurso = styled.div`
  ...
  display: flex;
  flex-direction: row;
  justify-content: ${({ onlyButtons }) =>
    onlyButtons ? "flex-end" : "space-between"};
  align-items: center;
  padding: 5px;
    ...
    <>
      <ContainerWrap>
        ...
        <ContainerInformacoesCurso onlyButtons={!hasDetails}>
          ...
          {professor && (
            <div>
              <h3>Professor:</h3> <h2>{professor}</h2>
            </div>
          )}
          {status && (
            <div>
          ...
          <DivBotao onlyButtons={!hasDetails}>
