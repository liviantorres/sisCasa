...
const HomeServidor = () => {
  ...
  const fetchAtividades = async () => {
    ...
    try {
      ...
      const atividadesFiltradas = response.data.filter(
        (atividade) => atividade.professorId !== parseInt(userId)
      );
  ...
  const handleSave = async () => {
    try {
      await axios.post(
        `https://scasa.ufc.br/api/atividade/${atividadeSelecionada.id}/aluno/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
